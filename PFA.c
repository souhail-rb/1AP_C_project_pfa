...
// Structure responsable de représenter un patient
typedef struct {
    int id;
...
// Structure responsable de création de dossier patient
typedef struct {
    int id;
    int patientId;
...
int dernierIdPatient = 0; // Variable globale pour suivre le dernier ID de patient attribué
...
void ajouterNouveauPatient(Patient patients[], int *nombrePatients) {
    ...
    do
    {
        ...
        // Vérification de l'âge pour s'assurer qu'il est dans la plage spécifiée
        if (nouveauPatient.age < 1 || nouveauPatient.age > 150) {
            printf("\033[0;31m");
            printf("\n\t\t\t\t\t\t\t :( L'age doit etre compris entre 1 et 150 ans, reessayer !\n\n");
            printf("\033[0m");
        }
    } while (nouveauPatient.age < 1 || nouveauPatient.age > 150); // Répéter la saisie si l'âge n'est pas valide

    do
    {
        printf("\t\t\t\t\t\tEntrez le sexe du patient (M/F) : ");
        scanf(" %c", &nouveauPatient.sexe);
        // Vérification du sexe pour s'assurer qu'il est 'M' ou 'F'
        if (toupper(nouveauPatient.sexe) != 'M' && toupper(nouveauPatient.sexe) != 'F') {
            printf("\033[0;31m");
            printf("\n\t\t\t\t\t\t\t :( Le sexe doit etre 'M' ou 'F',reessayer!\n\n");
            printf("\033[0m");
        }
    } while (toupper(nouveauPatient.sexe) != 'M' && toupper(nouveauPatient.sexe) != 'F'); // Répéter la saisie si le sexe n'est pas valide
...
// Fonction responsable de création de dossier patient
...
void supprimerDossierPatient(PatientRecord dossiers[], int *nombreDossiers) {
    ...
    for (int i = 0; i < *nombreDossiers; i++) {
        if (dossiers[i].id == idDossier) {
            // Déplacement des dossiers suivants pour combler le vide
